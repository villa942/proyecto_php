...
class UsuarioLoginDao{
    ...
    public function registrarUsuarioadmin(UsuarioLoginDto $usuarioLoginDto){
        $cnn=Conexion::getConexion();
        $mensaje ="";
        try {
            $query = $cnn->prepare("INSERT INTO usuarios values (?,?,?,?,?,?,?,?)");
    ...
    public function modificarUsuario(UsuarioLoginDto $usuarioLoginDto){
        ...
        $mensaje = "";
        try {
            $query= $cnn->prepare('UPDATE usuarios SET nombre=?, apellido=?, correo_electronico=?, cedula=?, celular=?, contraseña=?, tipo_usuario=? WHERE id_usuario=?');
            
            $query->bindParam(1,$usuarioLoginDto->getNombre());
            $query->bindParam(2,$usuarioLoginDto->getApellido());
            $query->bindParam(3,$usuarioLoginDto->getCorreo());
            $query->bindParam(4,$usuarioLoginDto->getCedula());
            $query->bindParam(5,$usuarioLoginDto->getCelular());
            $query->bindParam(6,$usuarioLoginDto->getContraseña());
            $query->bindParam(7,$usuarioLoginDto->getTipousuario());
            $query->execute();
    ...
    public function obtenerUsuario($idUsuario){
        ...
        $mensaje = "";
        try {
            $query= $cnn->prepare('SELECT * FROM usuarios WHERE id_usuario = ?');
    ...
    public function eliminarUsuario($idUsuario){
        ...
        $mensaje = "";
        try {
            $query = $cnn->prepare("DELETE FROM usuarios WHERE id_usuario= ?");
            $query->bindParam(1, $idUsuario);
            $query->execute();
